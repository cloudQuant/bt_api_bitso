...
class BitsoRequestTickerData(TickerData):
    ...
    def init_data(self) -> BitsoRequestTickerData:
        ...
        if isinstance(self.ticker_data, dict):
            if self.ticker_data.get("success"):
                payload = self.ticker_data.get("payload", {})
            else:
                payload = self.ticker_data.get("payload", {})

        if payload:
            self.ticker_symbol_name = payload.get("book")
            self.last_price = from_dict_get_float(payload, "last")
            self.bid_price = from_dict_get_float(payload, "bid")
            self.ask_price = from_dict_get_float(payload, "ask")
            self.volume_24h = from_dict_get_float(payload, "volume")
            self.high_24h = from_dict_get_float(payload, "high")
            self.low_24h = from_dict_get_float(payload, "low")
