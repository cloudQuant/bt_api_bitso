...
class BitsoBalanceData(BalanceData):
    def __init__(
        self,
        balance_info: Any,
        symbol_name: str,
...
class BitsoRequestBalanceData(BitsoBalanceData):
    def init_data(self) -> None:
        ...
        payload = (
            self.balance_data.get("payload", {})
            if isinstance(self.balance_data, dict)
            else self.balance_data
        )
        self.currency = from_dict_get_string(payload, "currency")
        self.available_balance = from_dict_get_float(payload, "available")
        self.locked_balance = from_dict_get_float(payload, "locked")
